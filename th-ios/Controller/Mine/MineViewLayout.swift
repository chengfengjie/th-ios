...
protocol TopicArticleSwitchHeaderLayout {
    var topicArticleSwitchHeader: UIView { get }
    func handleTopicArticleSwitchItemClick(itemIndex: NSNumber)
}
extension TopicArticleSwitchHeaderLayout where Self: MineViewController {
    ...
    func makeTopicArticleSwitchHeader() -> UIView {
        ...
        let bottomline = UIView().then {
            header.addSubview($0)
            $0.frame = CGRect.init(x: 15, y: self.topicArticleSwitchHeaderSize.height - 2,
                                   width: 36, height: 2)
            $0.backgroundColor = UIColor.pink
        }
